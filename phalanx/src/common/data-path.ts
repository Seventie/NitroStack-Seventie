...
export function loadDataJson<T>(filename: string, fallback: T): T {
  ...
  if (!resolved) {
    console.warn(`[data] ${filename} not found in any known data directory — using fallback.`);
    return fallback;
  }
  ...
  try {
    ...
    console.warn(`[data] ${filename} failed to parse (${(err as Error).message}) — using fallback.`);
    return fallback;
  }
}
