...
export class RiskService {
  ...
  private async runAgentWithLlm(
    ...
    const clauseBlock = subgraph.clauses
      .map((c) => {
        const txt = c.text.length > 500 ? c.text.slice(0, 500) + '... [trimmed for high-speed agent evaluation]' : c.text;
        return `<clause id="${c.id}" category="${c.category}" title="${c.heading}" obligor="${c.obligor ?? 'unspecified'}" obligee="${c.obligee ?? 'unspecified'}">\n${txt}\n</clause>`;
      })
      .join('\n\n');
